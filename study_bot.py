...
# Student Services (PDF names exactly as in /pdfs)
SERVICE_FILES = [
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        WELCOME_TEXT, parse_mode=ParseMode.HTML, reply_markup=build_main_keyboard()
    )
...
async def select_major(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Executive programs → send single plan directly
...
# ---- Course Details (تفاصيل المقررات) ----
...
async def inquiry_course(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.exists(COURSE_XLSX):
        await update.message.reply_text(
            "⚠️ ملف تفاصيل المقررات غير موجود.", reply_markup=build_main_keyboard()
        )
        return CHOOSING_MENU
    ...
    try:
        ...
        for _, row in matched.iterrows():
            crn_line = f"🔢 الرقم المرجعي: {row[crn_col]}\n" if crn_col in row.index and pd.notna(row[crn_col]) else ""
            msg = (
                f"{crn_line}"
...
# ---- Student Services (دليل خدمات الطالب) ----
async def service_manual(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not manuals:
        await update.message.reply_text(
            "⚠️ لا توجد ملفات متاحة حاليًا.", reply_markup=build_main_keyboard()
        )
        return CHOOSING_MENU
...
async def faculty_offices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if path:
        fname = os.path.basename(path) or "office_number"
        with open(path, "rb") as f:
...
async def cooperative_training(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not files:
        await update.message.reply_text(
            "⚠️ لا توجد ملفات للتدريب التعاوني.", reply_markup=build_main_keyboard()
        )
        return CHOOSING_MENU
    ...
    conv = ConversationHandler(
        ...
        states={
            CHOOSING_MENU: [
                ...
                MessageHandler(filters.Regex("^📝 وصف المقررات$"), describe_courses),
